...
########################################
# The following 3 lines are to remove the columns with ZERO varience in the Data
variance = df.var()

# Create a list of columns to keep (non-zero variance)
cols_to_keep = variance[variance != 0].index.tolist()

# Filter the DataFrame to keep these columns which has NOT zero Variance
...
Ef_1percent, auc_avg, auc_std, fp_mapped, fn_mapped = binary_classification_with_svm(f'{file_stem}', data, label)
